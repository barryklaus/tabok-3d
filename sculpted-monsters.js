...
export function sculptMajor(){
  ...
  root.userData.update=t=>{
    const summon=mode==='summon',moving=mode==='move'||mode==='levitate',levitating=!summon;body.position.y=levitating?.08+Math.sin(t*1.7)*.055:0;body.rotation.set(0,0,0);hips.rotation.set(0,0,0);torso.rotation.set(0,Math.sin(t*.53)*.02,0);head.rotation.set(0,Math.sin(t*.61)*.06,0);cape.rotation.set(Math.sin(t*.9)*.027+(moving?.12:0),0,0);

  ...
  class TabokWebGLBoard {
    ...
    paintHex(ctx, cell, images) {
      ...
      if (playable) {
        ...
        const depth = ctx.createLinearGradient(cell.x, cell.y - 36, cell.x, cell.y + 50);
        depth.addColorStop(0, '#3f2b1d');
        depth.addColorStop(.55, '#20140f');
        depth.addColorStop(1, '#080605');
        ctx.fillStyle = depth;
      ...
      if (playable) {
        ...
        ctx.filter = 'none';
        ctx.fillStyle = cell.type === 'T' ? 'rgba(10,82,73,.18)' : cell.type === 'P' ? 'rgba(104,35,124,.18)' : 'rgba(218,195,148,.07)';
      ...
      ctx.stroke();
      if (playable) {
        ctx.strokeStyle = 'rgba(255,222,150,.24)';
        ctx.lineWidth = 1.65;
    ...
    setQuality(quality) {
      ...
      const scale = ultra ? (mobile ? .86 : 1) : quality === 'full' ? (mobile ? .86 : 1) : quality === 'lite' ? (mobile ? .58 : .68) : this.preferredScale;
      this.setRenderScale(scale);
      this.motes.forEach((mote, index) => { mote.visible = !ultra && (quality !== 'lite' || index < 4); });
      this.portal.motes.forEach((mote, index) => { mote.visible = !ultra && (quality !== 'lite' || index < 4); });
      if (ultra) {
        this.app.render();
        this.app.ticker.stop();
        this.canvas.dataset.renderMode = 'static';
      } else {
        delete this.canvas.dataset.renderMode;
